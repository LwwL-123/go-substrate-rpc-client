...
import (
	...
	"github.com/LwwL-123/go-substrate-rpc-client/v3/xxhash"
)
...
func CreateStorageKey(meta *Metadata, prefix, method string, args ...[]byte) (StorageKey, error) {
	...
	if entryMeta.IsNMap() {
		return createNMapKey(meta, method, prefix, stringKey, entryMeta, args)
	}
...
func createKeyDoubleMap(meta *Metadata, method, prefix string, stringKey, arg, arg2 []byte,
	...
	hashers, err := entryMeta.GetHashers()
	if err != nil && len(hashers) != 2 {
		return nil, err
	}
...
func createKey(meta *Metadata, method, prefix string, stringKey, arg []byte, entryMeta StorageEntryMetadata) (
	...
	hashes, err := entryMeta.GetHashers()
...
func createNMapKey(meta *Metadata, method, prefix string, stringKey []byte, entryMeta StorageEntryMetadata, args [][]byte) (StorageKey, error) {
	hashers, err := entryMeta.GetHashers()
	if err != nil {
		return nil, err
	}
	if len(args) != len(hashers) {
		return nil, fmt.Errorf("len hasher num is not equal len args")
	}
	key := createPrefixedKey(method, prefix)
	for i, h := range hashers {
		_, err := h.Write(args[i])
		if err != nil {
			return nil, err
		}
		arg := h.Sum(nil)
